...
另外还存在一个特别的标记名 `tpb-root`（无值），用来标记可能被视作模板根的位置，这样，渲染就可以从任意位置开始了（即单独的局部渲染）。

> 注记：
> 局部渲染是针对DOM中的克隆节点的，但克隆节点上并不存在渲染配置。`tpb-root` 建立了克隆节点与其原始模板节点的映射。
> 当局部渲染时，通过克隆节点检索到原始模板节点，即可获取渲染配置执行渲染并替换。
>
> 因此，如果需要局部渲染，目标根节点必须标记 `tpb-root`，这样才会有效（注意名称不是 `tpl-` 前缀）。
...
**另外**，在当前渲染环境下还存在一个顶级的变量 `$$`，它引用初始传入的根数据，下级任意位置都可以通过该变量向下引用任何成员。如在上面的数据结构中，也可以通过 `$$.language` 引用语言信息而不论当前域处于哪一级。
...
每个循环中的当区域的父域引用 `$` 是数组所在的域（即 `data-for` 定义所在的域），可能不是数组本身，除非数组本身就是当前域（即：`tpb-for="$"`）。
