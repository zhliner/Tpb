...
以模板为驱动，功能指令书写在HTML元素上，自动解析完成事件的绑定和行为序列的编译。**注**：详见 `./template.md` 文档，这里只是简述。
...
另外，支持一个 `obt-src` 属性导入外部的 `On/By/To` 定义，文件为JSON格式（`{ on, by?, to? }`），同样，其中By和To是可选的。JSON中的配置对象可以是单个对象，也可以是一个配置对象的数组，这样就无需将多个配置以分号合并在一起了（更直观）。

`obt-src` 属性值支持逗号分隔的多个配置文件并列，如：`obt-src="obts/111.json, obts/aaa.json"`。并列的各组路径独立，都相对于模板根目录。
